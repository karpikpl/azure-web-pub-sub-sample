...
app.Logger.LogInformation("WebPubSub server url: {webPubSubServerUrl}", webPubSubServerUrl);
...
app.MapPost("/jobs", [Topic("jobspubsub", "jobs")] async (ILogger<Program> logger, Job job, CancellationToken cancellationToken) =>
{
    ...
    Uri webPubSubServerUri = new Uri($"{webPubSubServerUrl}/negotiate/{username}/{job.CorrelationId}");
